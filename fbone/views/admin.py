...
def set_polls_nodes(polls, nodes):
    ...
    return render_template('admin_set_poll_items_node.html', form=form,
                           current_user=current_user)
...
def set_polls_projects(polls, projects):
    ...
    polls_choices = [(poll.id, poll.name) for poll in polls]
    projects_choices = [(project.id, project.activation_key)
    ...
    if request.method == 'POST':
        ...
        db.session.commit()
        return redirect(form.next.data or url_for('admin.project_list'))
    return render_template('admin_set_poll_items_project.html', form=form,
                           current_user=current_user)
...
def polls(id=None):
    ...
    if project:
        ...
        if id:
            node = PollItem.query.filter_by(id=id).first()
            tree = db.session.query(PollItem.id, PollItem.name).\
                    filter_by(parent=node).all()
        else:
...
@login_required
@admin_required
def remove_poll(project_id, poll_id):
    project = Project.query.filter_by(id=project_id).first()
    poll = PollItem.query.filter_by(id=poll_id).first()
    project.remove_poll(poll)
    db.session.commit()
    return redirect('/admin/poll/' + project_id)


def set_sessions(sessions, projects):
    if len(sessions) > 1:
        sessions_from_today = [session for session in sessions if
                                session.begin > datetime.datetime.now()]
    else:
        sessions_from_today = sessions
    form = SetSessionForm(request.form)
    sessions_choices = [(session.id, session.begin)
                        for session in sessions_from_today]
    projects_choices = [(project.id, project.activation_key)
                        for project in projects]
...
def offers(id=None):
    ...
    if project:
        ...
        if id:
            node = Offer.query.filter_by(id=id).first()
            tree = db.session.query(Offer.id, Offer.name).\
                filter_by(parent=node).all()
        else:
            tree = [(x.id, x.jsonify_full())
                    for x in Offer.query.filter_by(depth=0)]
    return jsonify(tree)
...
def offers_types(type=1):
    ...
    if project:
        ...
        if(str(type) in offer_types):
            nodes = [offer for offer in offers if
                    (offer.depth == 0 and str(offer.type) == str(type))]
            tree = [(x.id, x.jsonify_full()) for x in nodes]
    return jsonify(tree)
...
@admin_required
def group_list():
    return render_template('list.html', title="Grupos",
        objects=Group.query.all(), active='group_list',
        actions=[['Editar', "edit", 'icon-pencil'],
        ['Borrar', "del", 'icon-trash']], current_user=current_user)
...
def page_list():
    ...
    return render_template('list.html', title=u"Páginas", objects=objects,
                            active='page_list', fields=['name'],
                            actions=[['Editar', "edit", 'icon-pencil'], ],
                            current_user=current_user)
...
def act_list():
    ...
    return render_template('list.html', title=u"Actos de Graduación",
                            objects=objects, active='act_list',
                            actions=[['Borrar', "del", 'icon-trash']],
                            fields=["password", ], current_user=current_user)
...
def set_session_term(term):
    ...
    projects = db.session.query(Project).\
        filter(Project.name != '0', Project.term == term).all()
    return set_sessions(sessions, projects)
...
def del_act(id):
    ...
    dir_path = os.path.join(os.path.join(admin.root_path, '../static/acts/'),
        act.password_hash)
...
def new_act():
    ...
    if request.method == 'POST':
        ...
        if file:
            ...
            dir_path = os.path.join(os.path.join(admin.root_path,
                '../static/acts/'), password_hash)
...
def new_poll_item(id=None):
    ...
    if request.method == 'POST':
        ...
        if not form.id.data or form.copy.data:
            poll_item = PollItem(name=form.name.data,
        ...
        if file:
            filename = secure_filename(file.filename)
            file.save(os.path.join(os.path.join(admin.root_path,
                '../static/poll_items/'), filename))
            ...
            if form.copy.data or form.id.data:
                ...
                return redirect(form.next.data
                    or url_for('admin.poll_item_list'))
...
def poll_item_list(id=None):
    ...
    if id:
        ...
        actions = [['Asignar Proyectos', "set", 'icon-hand-right'],
                    ['Ver', "view", 'icon-eye-open'],
...
def uploaded_file(id):
    ...
    return render_template("template.html", name=offer.name,
    ...
    if form.validate_on_submit():
        ...
        if not form.project_id.data or form.project_id.data == '':
            project = Project.query.\
                      filter_by(term=form.term.data, name='0').first()
            if not project:
...
def new_group():
    ...
    if form.validate_on_submit():
        ...
        if not form.group_id.data or form.group_id.data == '':
            if parent:
                node = Group(name=name, activation_key=activation_key,
                             depth=depth, type=type, parent_id=parent.id)
            else:
                node = Group(name=name, activation_key=activation_key,
...
def groups():
    if 'parent' in request.values:
        ...
        tree = db.session.query(Group.id, Group.name).\
                            filter_by(parent=node).all()
    else:
