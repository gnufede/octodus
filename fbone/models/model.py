...
class PollSelection(db.Model):
    __tablename__ = "polls_users"
    project_id = db.Column(db.Integer, ForeignKey("projects.id"),
        primary_key=True)
    user_id = db.Column(db.Integer, ForeignKey("users.id"), primary_key=True)
    poll_item_id = db.Column(db.Integer, ForeignKey("poll_items.id"),
        primary_key=True)
    poll_type = db.Column(db.Integer, ForeignKey("poll_items.type"),
        primary_key=True)
    date = db.Column(db.DateTime, default=db.func.now())
    project = relationship("Project", backref=backref("poll_selection",
        cascade="all,delete,delete-orphan"))
    user = relationship("User", backref=backref("poll_selection",
        cascade="all,delete,delete-orphan"))
    poll_item = relationship("PollItem",
        primaryjoin="PollItem.id==PollSelection.poll_item_id",
        backref=backref('poll_selection', cascade='all,delete,delete-orphan'))
...
class PollItem(db.Model):
    ...
    def jsonify_full(self):
        return {'id': self.id, 'name': self.name,
            'img': url_for('static', filename="polls/" + self.picture),
...
class OfferSelection(db.Model):
    ...
    offer_type = db.Column(db.Integer, ForeignKey("offers.type"),
        primary_key=True)
    date = db.Column(db.DateTime, default=db.func.now())
    project = relationship("Project", backref=backref("offer_selection",
        cascade="all,delete,delete-orphan"))
    user = relationship("User", backref=backref("offer_selection",
        cascade="all,delete,delete-orphan"))
    offer = relationship("Offer",
        primaryjoin="Offer.id==OfferSelection.offer_id",
        backref=backref('offer_selection', cascade='all,delete,delete-orphan'))
...
class Offer(db.Model):
    ...
    def jsonify(self):
        return {'id': self.id, 'name': self.name,
            'children': [x.jsonify() for x in self.children]}
    ...
    def jsonify_full(self):
        if self.price:
            ...
            jprice = 'N/A'
        return {'id': self.id, 'name': self.name,
            'img': url_for('static', filename="offers/" + self.picture),
            'description': self.description, 'price': jprice,
            'children': [x.jsonify() for x in self.children]}
...
class Appointment(db.Model):
    ...
    project = relationship("Project", backref=backref("appointments",
        cascade="all,delete,delete-orphan"))
    user = relationship("User", backref=backref("appointments",
        cascade="all,delete,delete-orphan"))
    session = relationship("Session", backref=backref('appointments',
        cascade='all,delete,delete-orphan'))
...
class Project(db.Model):
    ...
    sessions = relationship("Session", secondary=projects_sessions,
        backref="projects")
    offers = relationship("Offer", secondary=projects_offers,
        backref="projects")
    poll_items = relationship("PollItem", secondary=projects_poll_items,
        backref="projects")
...
#        self.term = term
    def project_already_created(self, activation_key):
        return len(db.session.query(Project).
            filter(Project.activation_key == activation_key).all())

    def create(self, node):
        activation_key = node.activation_key + self.term
        new_project = db.session.query(Project).\
            filter(Project.activation_key == activation_key).first()
        if not new_project:
            ...
            new_project.poll_items = node.poll_items
        if node.parent:
            parent_activation_key = node.parent.activation_key + self.term
            parent_project = db.session.query(Project).\
                filter(Project.activation_key == parent_activation_key).first()
            if parent_project:
...
class Node(db.Model):
    ...
    #projects = relationship("Project", backref="node")
    poll_items = relationship("PollItem", secondary=nodes_poll_items,
        backref="nodes")
    ...
    def jsonify(self):
        ...
        #return "{'name':%s, 'id':%d, 'children':%s}" %\
        # (self.name, self.id, repr([x.jsonify() for x in self.children]))

        #return jsonify(name=self.name, id=self.id,
        #children=str([x.jsonify() for x in self.children]))
