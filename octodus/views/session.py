...
def list(sessions, actions=None):
    ...
    if not actions:
        actions = [['Editar', "edit", 'icon-pencil'],
                    ['Asignar a Proyectos', "set", 'icon-hand-right'],
                    ['Ver citas', "view", 'icon-eye-open'],
    ...
    return render_template('session_index.html', sessions=sessions2,
                            fields=['id', 'fecha_de_sesion', 'inicio', 'fin',
                                    'block_capacity', 'block_duration',
                                     'citas'],
                            actions=actions, current_user=current_user)
...
def list_project(project_id):
    ...
    actions = [['Quitar del proyecto',
                "del/" + project_id + '/session',
                'icon-trash']]
...
def new_session():
    ...
    today_str = str(date_today.year) + "-" + str(date_today.month) + "-" +\
                 str(date_today.day)
    ...
    return render_template('session_new.html', form=form,
                           current_user=current_user, today=today_str,
                           duration=10, capacity=4)
...
def new_session_post():
    ...
    if form.validate_on_submit():
        ...
        dates = [date for date in (begin_date + datetime.timedelta(days)
                    for days in range((end_date - begin_date).days + 1))
        ...
        for date in dates:
            if form.id.data and form.id.data != '':
                ...
                session = Session()
            session.begin = datetime.datetime(date.year, date.month, date.day,
                                            time_begin.hour, time_begin.minute)
            session.end = datetime.datetime(date.year, date.month, date.day,
...
def pub(id):
    ...
    today_str = str(session.begin.year) + "-" + str(session.begin.month) +\
                    "-" + str(session.begin.day)
    form.id.data = session.id
    return render_template('session_new.html', form=form, today=today_str,
                            current_user=current_user,
                            duration=form.block_duration.data,
                            capacity=form.block_capacity.data)
...
def view(id):
    session = Session.query.filter_by(id=id).first_or_404()
    appointments = db.session.query(Appointment.date, Project.activation_key,
    ...
    return render_template('list.html', objects=appointments,
                title="Citas para el " + session.begin.date().isoformat(),
                delete=[('session_id', -3), ('user_id', -2),
                        ('project_id', -1)],
                fields=['date', 'name', 'surname', 'email', 'activation_key'])
