...
class LoginForm(Form):
    ...
    email = TextField(_(u'Dirección de email'), [required()]) #Username or email address
...
class SignupForm(Form):
    ...
    surname = TextField(_(u'Apellidos'), [required(message='Quieres que tus apellidos aparezcan en la orla')]) #Surname
    ...
    def validate_email(self, field):
        if User.query.filter_by(email=field.data).first() is not None:
            raise ValidationError, gettext(u'Este email ya está registrado') #This email is taken


class RecoverPasswordForm(Form):
    email = TextField(_(u'Tu email'), validators=[ #Your email
                      email(message=_(u'Se requiere un email válido'))]) #A valid email address is required
...
class ChangePasswordForm(Form):
    ...
    password = PasswordField(u'Contraseña', validators=[ #Password
                             required(message=_(u'Se requiere una contraseña de al menos 6 caracteres')), #Password is required
                            length(min=6, max=56)])
    password_again = PasswordField(_(u'Contraseña otra vez'), validators=[ #Password again
